...
Celem projektu było stworzenie czterech Rubber Ducky. Każde z nich miało wykonywać inne zadanie, mianowicie:
\begin{itemize}
...
Kod pisaliśmy używając biblioteki DigiKeyboard.h. Bibilioteka ta znacznie ułatwiła nam pracę, ponieważ zawarte w niej funkcje pozwalają emulować działanie klawiatury. Dzięki użyciu tej biblioteki nie musimy również szukać wartości odpowiednich klawiszy w HID usage table, bo są one już w bibliotece zdefiniowane. Ważne jest też to, że funkcje w tej bibliotece pozwalają nam na emulowanie kilku klawiszy jednocześnie, ponieważ ze względu na to, że mamy jedynie klawiaturę, musimy używać skrótów klawiszowych systemu Windows, żeby móc zapisać i włączyć napisany przez nas skrypt na komputerze do którego podłączymy nasze Rubber Ducky.
W napisanych przez nas programach używamy funkcji delay, która każe skryptowi zaczekać przed wykonaniem dalszych instrukcji. Jest to ważne dla niezawodności działania Rubber Ducky. Niektóre komputery mogą potrzebować trochę więcej czasu na przetworzenie danych wysyłanych przez klawiaturę. Dzięki zaczekaniu chwili przed "wciśnięciem" kolejnych klawiszy mamy pewność, że poprzednia akcja zostanie obsłużona przed próbą wykonania kolejnej, dzięki czemu wszystkie wykonają się poprawnie. System może mieć również problem z przepustowością przetwarzania danych z klawiatury, przez co przy zbyt szybkim wysłaniu zbyt wielu poleceń, któreś może się nie wykonać. 
